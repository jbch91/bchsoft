...
export class QuickGuidesComponent {
  ...
  get brandModelOptions(): BrandModelOption[] {
    const map = new Map<string, BrandModelOption>();
  ...
  async onClientChange(): Promise<void> {
    this.resetForm();
    this.refreshViewSoon();
    await Promise.all([this.loadGuides(), this.loadAssets()]);
  }
  ...
  onBrandModelChange(): void {
    const option = this.brandModelOptions.find((item) => item.key === this.selectedBrandModelKey);
    if (!option) return;
    this.form.brand = option.brand;
    this.form.model = option.model;
    this.ensureDocumentCode();
    if (!this.form.equipmentName.trim()) {
      this.form.equipmentName = option.equipmentName;
    }
  }

  onBrandModelManualChange(): void {
    this.ensureDocumentCode();
  }
  ...
  async deleteGuide(guide: QuickGuideDto): Promise<void> {
    if (!this.selectedClientId) return;
    const ok = window.confirm(`¿Eliminar la guía rápida de ${guide.brand} / ${guide.model}?`);
    if (!ok) return;
  ...
  private ensureDocumentCode(): void {
    ...
    const code = this.buildDocumentCode(this.form.brand, this.form.model);
    if (code) this.form.documentCode = code;
  }
